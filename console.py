...
def addPosition():
    ...
    while choice.lower() != "no":
        choice = input(
            "Do you wish to add another definition of the word? Type yes/no: ")
        if choice.lower() == "yes":
            value = input("Insert definition: ")
            data[key].append(value)
...
def delPosition():
    key = input(
        "Insert the word you want to delete (coresponding definitions will be deleted as well): ")
    if key in data:
        del data[key]
    else:
        print("The word doesn't appear to be in a dictionary.")
...
def main():
    ...
    while choice != "6":
       ...
       if choice == "6" and not ifSaved:
           quitMenu = input(
               "You haven't saved your dictionary. If you are sure you want to quit type 'yes': ")
